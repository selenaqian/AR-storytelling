...
public class bridge_inScene : MonoBehaviour
{
    ...
    if(valid && goldilocks.activeSelf) {
        ...
        if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began)
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity))
            {
                ...
                if(obj == bridge)
                {
                    ...
                    allObjects[4].targetPrefab = brokenBridge;
                    worldScripts.GetComponent<ActiveActions>().bridgeBroken = true;
        ...
        {
            Debug.Log("goldilocks ran into bridge");
            allObjects[4].targetPrefab = brokenBridge;
            worldScripts.GetComponent<ActiveActions>().bridgeBroken = true;
