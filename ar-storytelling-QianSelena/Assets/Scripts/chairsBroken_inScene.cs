...
public class chairsBroken_inScene : MonoBehaviour
{
    ...
    {     
        ...
        if(brokenMessage.text == "Oh no! The chairs broke...\nTry to fix?")
        {
            if (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began)
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity))
                {
                    ...
                    if(obj == brokenChairs)
                    {
                        Debug.Log("hit broken chairs");
                        allObjects[0].targetPrefab = chairs;
                        worldScripts.GetComponent<ActiveActions>().chairsBroken = false;
                        brokenMessage.text = "";
        ...
        //TODO: make this message go away after a bit + get replaced by try to fix? - do it in the chairsBroken_inScene script! and then that should go away after a bit too
        //things it needs to do:
        //if fix message is up, check for touch broken chairs then swap back to chairs and set chairsBroken to false
    }
}
